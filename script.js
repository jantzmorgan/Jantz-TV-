...
let invidiousRetryCount = 0;

function getInvidiousEmbedUrl(videoId, autoplay) {
	const base = INVIDIOUS_INSTANCES[invidiousInstanceIndex % INVIDIOUS_INSTANCES.length];
	const ap = autoplay ? 1 : 0;
	return base + '/embed/' + videoId + '?autoplay=' + ap + '&local=true&quality=dash';
}

function getYouTubeEmbedUrl(videoId, autoplay) {
	const ap = autoplay ? 1 : 0;
	return 'https://www.youtube.com/embed/' + videoId + '?rel=0&playsinline=1&autoplay=' + ap;
}

function rotateInvidiousInstance() {
	invidiousInstanceIndex = (invidiousInstanceIndex + 1) % INVIDIOUS_INSTANCES.length;
}
...
videoPlayer.addEventListener('error', function() {
	if (!USE_INVIDIOUS || !currentVideo || !currentVideo.videoId) {
		return;
	}
	if (invidiousRetryCount >= INVIDIOUS_INSTANCES.length) {
		videoPlayer.src = getYouTubeEmbedUrl(currentVideo.videoId, false);
		invidiousRetryCount = 0;
		return;
	}
	rotateInvidiousInstance();
	invidiousRetryCount++;
	videoPlayer.src = getInvidiousEmbedUrl(currentVideo.videoId, false);
});
...
function playVideo(video, addToHistory, scrollToPlayer, autoplay) {
	...
	invidiousRetryCount = 0;
	if (USE_INVIDIOUS) {
		videoPlayer.src = getInvidiousEmbedUrl(video.videoId, autoplay);
	} else {
		videoPlayer.src = getYouTubeEmbedUrl(video.videoId, autoplay);
	}
...
function setupAppLikeMobileBehavior() {
	...
	function clampAppScroll() {
		if (!app) {
			return;
		}
		const maxScroll = getMaxAppScroll();
		if (app.scrollTop < 0) {
			app.scrollTop = 0;
		}
		if (app.scrollTop > maxScroll) {
			app.scrollTop = maxScroll;
		}
	}
	...
	if (app) {
		...
		app.style.webkitOverflowScrolling = 'auto';
	}
	...
	document.addEventListener('touchend', function(event) {
		...
		clearSelection();
		clampWindowScroll();
		requestAnimationFrame(clampAppScroll);
	}, { passive: false, capture: true });
	if (!app) {
		return;
	}
	...
	app.addEventListener('touchmove', function(event) {
		...
		const maxScroll = getMaxAppScroll();
		const atTop = app.scrollTop <= 1;
		const atBottom = app.scrollTop >= maxScroll - 1;
