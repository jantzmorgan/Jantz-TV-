...
// Stronger PWA app-like scroll/copy/zoom lock
// ===============================
...
function playVideo(video, addToHistory = true, scrollToPlayer = false, autoplay = false) {
	...
	videoPlayer.src = `https://www.youtube.com/embed/${video.videoId}?rel=0&playsinline=1&autoplay=${autoplayValue}`;
...
function scrollToFavorites() {
	...
	app.scrollTo({
		top: Math.max(0, favoritesHeading.offsetTop - 10),
		behavior: "smooth"
	});
}
...
function setupAppLikeMobileBehavior() {
	...
	window.addEventListener("scroll", clampWindowScroll, {
		passive: true
	});
