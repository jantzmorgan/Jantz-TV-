...
const channels = [{
	...
	},
	{
		...
		tagline: "Manual completed livestreams only.",
		channelNumber: "CH 09",
		handle: "@RealCandaceO",
...
const youtubeSearchBox = document.getElementById("youtubeSearchBox");
...
function isManualExpensiveChannel(channel) {
	return channel && channel.feedType === "completedLives";
}
...
function filterAndFormatVideos(items, channel) {
	return items
		.filter(function(video) {
			...
			const embeddable = !(video.status && video.status.embeddable === false);
...
function updateHeaderForHome() {
	...
	screenSubtitle.textContent = "Latest videos.";
	viewModeLabel.textContent = "Home Feed";
	channelName.textContent = "Latest Videos";
	channelTagline.textContent = "Newest 5+ minute videos.";
	recentVideosTitle.textContent = "Latest Videos";
}
...
async function loadHomeFeed(forceRefresh) {
	...
	updateHeaderForHome();
	currentLoadedVideos = [];
	setLoadingMessage("Loading newest videos...");
	try {
		const homeChannels = channels.filter(function(channel) {
			return !isManualExpensiveChannel(channel);
		});

		const results = await Promise.allSettled(
			homeChannels.map(function(channel) { return fetchVideosForChannel(channel, VIDEO_LIMIT, forceRefresh); })
		);
		...
		results.forEach(function(result, index) {
			if (result.status === "fulfilled") {
				...
				console.log("Channel failed:", homeChannels[index].name, result.reason);
			}
		});
		...
		if (homeVideos.length > 0) {
			playVideo(homeVideos[0], false, false, false);
		} else {
			videoPlayer.src = "";
...
async function loadChannel(channelIndex, forceRefresh, allowExpensiveSearch) {
	if (forceRefresh === undefined) forceRefresh = false;
	if (allowExpensiveSearch === undefined) allowExpensiveSearch = false;
	...
	if (isManualExpensiveChannel(channel) && !allowExpensiveSearch) {
		renderManualCompletedLivesPrompt(channel);
		return;
	}
	...
	try {
		...
		renderVideos(videos);
		if (videos.length > 0) {
			playVideo(videos[0], false, false, false);
		} else {
			videoPlayer.src = "";
			clearCurrentVideoDisplay();
			if (channel.feedType === "completedLives") {
...
function renderManualCompletedLivesPrompt(channel) {
	recentVideosTitle.textContent = channel.channelNumber + " Manual Load";
	recentVideos.innerHTML = "";

	const card = document.createElement("div");
	card.classList.add("manual-load-card");

	const copy = document.createElement("div");
	copy.classList.add("manual-load-copy");

	const label = document.createElement("p");
	label.classList.add("tiny-label");
	label.textContent = "Quota Saver";

	const title = document.createElement("h3");
	title.textContent = "Candace lives are manual.";

	const note = document.createElement("p");
	note.textContent = "Completed livestream search is expensive, so it will not run from Home, Guide, arrows, Refresh, or app startup.";
	...
	button.type = "button";
	button.textContent = "Load Candace Lives";
	button.addEventListener("click", function() {
		loadChannel(currentChannelIndex, true, true);
	});
...
async function performYouTubeSearch() {
	...
	if (!query) {
		alert("Type something to search first buddy.");
		return;
	}

	currentView = "search";
	currentLoadedVideos = [];
	channelNumber.textContent = "SEARCH";
	screenSubtitle.textContent = "Manual YouTube search.";
	viewModeLabel.textContent = "Manual Search";
	channelName.textContent = "Search YouTube";
	channelTagline.textContent = "Search videos or accounts. Only runs after pressing Search.";
	recentVideosTitle.textContent = "Search Results";
	setLoadingMessage("Searching YouTube manually...");
	...
	try {
		...
		setEmptyMessage(recentVideos, "Search failed. Check quota/API key/console.");
	}
}

async function fetchYouTubeSearchResults(query) {
	const cacheKey = query.toLowerCase();
	if (youtubeSearchCache[cacheKey]) {
	...
	if (videoIds.length > 0) {
		const detailsUrl = "https://www.googleapis.com/youtube/v3/videos?key=" + API_KEY +
			"&id=" + videoIds.join(",") +
	...
	items.forEach(function(item) {
		if (item.id && item.id.videoId && videoMap[item.id.videoId]) {
			const formattedVideo = formatSearchVideo(videoMap[item.id.videoId]);
			if (formattedVideo) {
				results.push(formattedVideo);
			}
			return;
		}

		if (item.id && item.id.channelId) {
			results.push(formatSearchChannel(item));
		}
	});

	youtubeSearchCache[cacheKey] = results;
	return results;
}

function formatSearchVideo(video) {
	...
	return {
		...
		channelNumber: "SEARCH",
		channelTagline: "Manual YouTube search",
		handle: "",
		feedType: "search",
		publishedAt: video.snippet.publishedAt || "",
		thumbnail: getBestThumbnail(video),
		durationSeconds: parseDuration((video.contentDetails && video.contentDetails.duration) || ""),
		wasCompletedLive: Boolean(
			video.liveStreamingDetails &&
			video.liveStreamingDetails.actualStartTime &&
			video.liveStreamingDetails.actualEndTime
...
function formatSearchChannel(item) {
	...
	return {
		resultType: "channel",
		title: (item.snippet && item.snippet.title) || "YouTube Channel",
...
function renderSearchResults(results, query) {
	...
	if (!results || results.length === 0) {
		setEmptyMessage(recentVideos, "No playable videos or accounts found for “" + query + "”.");
		return;
	}

	const playableVideos = results.filter(function(result) {
		return result.resultType !== "channel";
	});
	currentLoadedVideos = playableVideos;

	results.forEach(function(result) {
		if (result.resultType === "channel") {
			recentVideos.appendChild(createSearchChannelRow(result));
		} else {
			recentVideos.appendChild(createVideoRow(result, { context: "search" }));
		}
	});
}

function createSearchChannelRow(channelResult) {
	const row = document.createElement("div");
	row.classList.add("video-item", "channel-result-item");

	const thumb = document.createElement("div");
	thumb.classList.add("video-thumb", "channel-result-thumb");

	if (channelResult.thumbnail) {
		const img = document.createElement("img");
		img.src = channelResult.thumbnail;
		img.alt = channelResult.title;
		img.loading = "lazy";
		thumb.appendChild(img);
	} else {
		thumb.textContent = "CH";
	}
	...
	title.textContent = channelResult.title;

	const channel = document.createElement("div");
	channel.classList.add("video-channel");
	channel.textContent = "YouTube account • tap to load recent videos";
	...
	action.type = "button";
	action.textContent = "CH";
	action.title = "Load this channel";
	...
	row.addEventListener("click", function() {
		loadSearchChannel(channelResult);
	});

	return row;
}

async function loadSearchChannel(channelResult) {
	if (!channelResult || !channelResult.channelId) {
		return;
	}

	const tempChannel = {
		name: channelResult.title,
		tagline: "YouTube account from manual search.",
		channelNumber: "SEARCH",
		channelId: channelResult.channelId,
		feedType: "uploads"
	};

	currentView = "searchChannel";
	currentLoadedVideos = [];
	channelNumber.textContent = "SEARCH";
	screenSubtitle.textContent = "Manual channel search.";
	viewModeLabel.textContent = "Search Channel";
	channelName.textContent = channelResult.title;
	channelTagline.textContent = "Recent 5+ minute videos from this account.";
	recentVideosTitle.textContent = channelResult.title;
	setLoadingMessage("Loading this account manually...");

	try {
		const videos = await fetchUploadVideos(tempChannel, VIDEO_LIMIT, true);
		currentLoadedVideos = videos;
		renderVideos(videos);

		if (videos.length === 0) {
			setEmptyMessage(recentVideos, "No 5+ minute embeddable videos found for this account.");
		}
	} catch (error) {
		console.log("Search channel load error:", error);
		setEmptyMessage(recentVideos, "Could not load this account. Check console.");
	}
}
...
function createVideoRow(video, options) {
	...
	row.appendChild(action);
	row.addEventListener("click", function() {
		playVideo(video, true, true, true);
	});
	return row;
}

function playVideo(video, addToHistory, scrollToPlayer, autoplay) {
	...
	videoPlayer.src = "https://www.youtube.com/embed/" + video.videoId + "?rel=0&playsinline=1&autoplay=" + ap;
...
function refreshCurrentView() {
	...
	if (isManualExpensiveChannel(channel)) {
		renderManualCompletedLivesPrompt(channel);
		return;
	}

	if (currentView === "search") {
		setEmptyMessage(recentVideos, "Manual search does not auto-refresh. Press Search again when you want to spend quota.");
		return;
	}
...
function playRandomVideo() {
	...
	playVideo(currentLoadedVideos[randomIndex], true, true, true);
}
...
function buildGuide() {
	...
	homeButton.innerHTML = "<span>HOME</span><strong>Latest Videos</strong><small>Newest 5+ minute videos.</small>";
...
function scrollToSearch() {
	const app = document.querySelector(".app");
	if (!app || !youtubeSearchBox) {
		return;
	}
	app.scrollTo({ top: Math.max(0, youtubeSearchBox.offsetTop - 10), behavior: "smooth" });
	if (youtubeSearchInput) {
...
if (bottomSearchBtn) bottomSearchBtn.addEventListener("click", scrollToSearch);
if (youtubeSearchBtn) youtubeSearchBtn.addEventListener("click", performYouTubeSearch);
if (youtubeSearchInput) {
	youtubeSearchInput.addEventListener("keydown", function(event) {
		if (event.key === "Enter") {
...
document.addEventListener("keydown", function(event) {
	...
	const typingInField = tagName === "input" || tagName === "textarea" || event.target.isContentEditable;
